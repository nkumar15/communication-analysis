# Developer & Operational Guides

Practical "How-to" guides for installing, deploying, and extending the platform.

## Operations
- [Deployment Guide](deployment.md): How to deploy to production.
- [Data Lifecycle](data-lifecycle.md): Managing soft-deletes and data retention.
- [Celery Setup](celery-setup.md): Background worker configuration.

## Testing
- [Automated Browser Testing](testing/automated-browser.md): Using the Playwright tool.
- [B2B Activation Test](testing/b2b-e2e-activation.md): Testing the critical onboarding flow.
- [Testing Workflows](testing/workflows.md): Manual and automated procedures.
