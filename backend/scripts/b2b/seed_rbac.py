...
1. Base boilerplate (base/ directory) - Always loaded
2. Use case configuration (use_cases/) - Based on USE_CASE env var
...
# Ensure /app is first in path for imports if running as script
# This allows imports like 'core.db.session' to work
current_file = Path(__file__).resolve()
# backend directory is 3 levels up from scripts/b2b/seed_rbac.py
backend_dir = current_file.parent.parent.parent

if str(backend_dir) not in sys.path:
    sys.path.insert(0, str(backend_dir))
    
# Also check for /app (Docker container path)
...
async def seed_sensitivity_levels(db: AsyncSession, config: dict) -> None:
    """Seed sensitivity levels from data_classification plugin config"""
    levels_data = config.get('sensitivity_levels', [])
    
    if not levels_data:
        print("  ⚠️  No sensitivity levels in data_classification config")
        return
    ...
    if not tenant:
        print("  ⚠️  No tenant found for sensitivity level seeding")
        return

    print(f"  🔒 Seeding {len(levels_data)} sensitivity levels...")
    seeded_count = 0
    ...
    for level_data in levels_data:
        result = await db.execute(
            select(SensitivityLevel).where(
                ...
                SensitivityLevel.name == level_data['name']
            )
        )
        existing = result.scalar_one_or_none()
    ...
    await db.flush()
    if seeded_count > 0:
        print(f"    ✓ Seeded {seeded_count} sensitivity levels")
    else:
        print("    ✓ Sensitivity levels already seeded")

async def seed_actions(db: AsyncSession) -> None:
    """Seed RBAC actions from base/actions.yaml"""
    data = load_yaml(BASE_DIR / 'actions.yaml')
...
async def seed_saas_resources(db: AsyncSession) -> None:
    """Seed base SaaS resources from base/saas_resources.yaml"""
    data = load_yaml(BASE_DIR / 'saas_resources.yaml')
...
async def seed_saas_roles(db: AsyncSession) -> None:
    """Seed base tenant role templates from base/saas_roles.yaml"""
    data = load_yaml(BASE_DIR / 'saas_roles.yaml')
...
async def initialize_plugins_if_enabled(db):
    """Load plugins from plugins.yaml (auto-detect) or RBAC_PLUGINS env var"""
    ...
    if enabled:
        ...
        # Fallback: Auto-detect based on config presence
        # Filter keys that are actual plugins (excluding random keys if any)
        # For now, assume top-level keys are plugin names
        plugin_names = list(plugin_config.keys())

    if not plugin_names:
        print("✓ No RBAC plugins enabled (none in plugins.yaml or RBAC_PLUGINS)")
        return
    
    print(f"\\n🔌 Initializing {len(plugin_names)} RBAC plugins: {', '.join(plugin_names)}...")
    ...
    if tenant:
        ...
        current_features = dict(tenant.features) if tenant.features else {}
        current_features['plugins'] = plugin_names
    ...
    for name in plugin_names:
        if name == 'geographic_boundaries':
            ...
            # Seed sensitivity levels
            await seed_sensitivity_levels(db, plugin_config.get(name, {}))
...
async def seed_b2b_rbac_data(db: AsyncSession) -> None:
    """
    Core seeding logic reusable by tests and main script.
    Assumes USE_CASE env var is set if needed.
    """
