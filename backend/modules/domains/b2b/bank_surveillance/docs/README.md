# Bank Surveillance Platform

> An enterprise-grade communication surveillance platform — demonstrated using the Enron dataset.

## Overview

This platform transforms raw communication data into actionable compliance insights. Unlike simple dataset explorers, it provides a complete surveillance workflow: from ingestion to alerts, investigations, cases, and regulatory reporting.

**Key Differentiators:**
- 🤖 Multi-Agent AI Analysis (Intent, Policy, Evasion detection)
- 🕸️ Social Graph Intelligence (Clique detection, centrality analysis)
- 🏢 Enterprise Multi-Tenant Architecture (Region/Team isolation)
- 📊 Production-Grade Audit Trail (Regulator-ready exports)

## Target Platform
- [x] Web
- [ ] Mobile (iOS/Android)
- [ ] Backend API Only

## Documentation

| Document | Description |
|----------|-------------|
| [Personas](./personas.md) | User personas and journeys |
| [Navigation](./navigation.md) | Information architecture |
| [Page Specs](./pages/) | Per-page specifications |
| [Wireframes](./wireframes/) | Visual designs |
| [Demo Scripts](./demos/) | Per-persona demos |
| [Technical](./technical/) | API, Schema, Architecture |

## Quick Links

### Product Docs
- [Personas](./personas.md) - Sarah (Exec), Marcus (Analyst), Priya (Risk)
- [Navigation IA](./navigation.md)

### Pages
- [Dashboard](./pages/dashboard.md)
- [Alerts](./pages/alerts.md)
- [Investigation Workspace](./pages/investigation.md)
- [Case Management](./pages/case_management.md)
- [Search & RAG](./pages/search_rag.md)
- [More pages...](./pages/)

### Technical Docs
- [API Reference](./technical/api.md)
- [Database Schema](./technical/schema.md)
- [Architecture](./technical/architecture.md)
