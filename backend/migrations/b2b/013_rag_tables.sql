...
CREATE TABLE IF NOT EXISTS b2b.embedding_cache (
...
CREATE TABLE IF NOT EXISTS b2b.rag_documents (
...
-- Indexes
CREATE INDEX idx_rag_documents_tenant ON b2b.rag_documents(tenant_id);
CREATE INDEX idx_rag_documents_status ON b2b.rag_documents(status);
...
-- 1. Enable RLS
ALTER TABLE b2b.rag_documents ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS rag_documents_isolation_policy ON b2b.rag_documents;
CREATE POLICY rag_documents_isolation_policy ON b2b.rag_documents
    USING (
