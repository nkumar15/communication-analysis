...
class TestDocumentStatus:
    ...
    @pytest.mark.asyncio
    async def test_get_status_after_upload(self, api_client: AsyncClient, nse_test_setup, mock_pdf_file):
        ...
        status_response = await api_client.get(
            f"/api/domain/nse/rag/status/{job_id}",
            headers={"Authorization": f"Bearer {nse_test_setup['token']}"}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_status_nonexistent_job(self, api_client: AsyncClient, nse_test_setup):
        """Test that querying nonexistent job_id returns 404"""
        fake_job_id = str(uuid4())
        
        response = await api_client.get(
            f"/api/domain/nse/rag/status/{fake_job_id}",
            headers={"Authorization": f"Bearer {nse_test_setup['token']}"}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_status_invalid_job_id_format(self, api_client: AsyncClient, nse_test_setup):
        """Test that invalid job_id format is rejected"""
        response = await api_client.get(
            "/api/domain/nse/rag/status/not-a-valid-uuid",
            headers={"Authorization": f"Bearer {nse_test_setup['token']}"}
        )
