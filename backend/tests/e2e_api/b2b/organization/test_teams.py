...
class TestTeamManagement:
    ...
    async def test_remove_user_from_last_team_allows_zero_memberships(
        ...
        # Step 1: Create a custom team (Engineering)
        team_resp = await api_client.post(
            "/api/b2b/teams/",
        ...
        custom_team_id = team_resp.json()["id"]
        
        # Step 2: Create a new user and add them ONLY to the custom team
        user = await create_test_user(
            session,
            tenant_id=setup["tenant_id"],
            email=f"orphan_test@{tenant.domain}",
            role_slug="member"
        )
        
        # Add user to custom team
        add_resp = await api_client.post(
            f"/api/b2b/teams/{custom_team_id}/members",
            json={"user_id": str(user.id), "team_role": "team_contributor"},
            headers={"Authorization": f"Bearer {token}"}
        )
        assert add_resp.status_code == 201
        
        # Step 3: Verify user is member of exactly 1 team (the custom team)
        from modules.b2b.services.team_service import get_user_teams
        user_teams_before = await get_user_teams(session, user.id)
        assert len(user_teams_before) == 1
        assert str(user_teams_before[0][0].id) == custom_team_id
        
        # Step 4: Remove user from the custom team
        remove_resp = await api_client.delete(
            f"/api/b2b/teams/{custom_team_id}/members/{user.id}",
            headers={"Authorization": f"Bearer {token}"}
        )
